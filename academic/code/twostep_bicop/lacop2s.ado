/*
 * @Author: Xuanlong.QIN
 * @Date: 2023-07-17 16:24:18 
 * @Last Update the command of run lacop1s model in lacop2s model
 * @Last Modified time: 2023-07-17 16:24:18 
 */
...
capture program drop lacop2s
program lacop2s, eclass
version 16
...
capture program drop twostep_mk2nd
program define twostep_mk2nd
	...
	// xuanlong 17/06/2023
	// this function only works when the model is lacop1s 
	if "`model'" == "lacop1s" {
		...
		display ". Warning: Only lacop1s is allowed as firstlevel model."
		exit	
	}
	...
	quietly {
		...
		// xuanlong 16/06/2023
		display "Weight parameter is: " "[`weight'`exp']"
...
capture program drop ParseWeight
program define ParseWeight, rclass
	syntax [anything][if] [in] [using] [fweight aweight pweight iweight] [,*]
...
program define GetVarlist, rclass
	...
	display "indpendent variables are: " "`indepvar'"
...
capture program drop twostep_unitcpr
...
capture program drop twostep_clustercpr
...
* Unitregby-plot
capture program drop twostep_unitregby
...
capture program drop twostep_dot
...
* Fallback mode
capture program drop twostep_fallback
...
* edv_OLS
capture program drop twostep_ols
...
* edv_WLS
capture program drop twostep_wls
...
* edv_borjas
capture program drop twostep_borjas
...
* edv_FGLS-1
capture program drop twostep_fgls1
...
* edv_FGLS-2
capture program drop twostep_fgls2
...
capture program drop ParseSecond
...
* Parse By-String
capture program drop ParseByString
...
capture program drop GrepIfVarnames
